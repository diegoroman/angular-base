(function(ng) {
    ...
    Module.directive('ngCustomDatepicker', ['$timeout', function($timeout) {
		...
		return {
			...
			link: function (scope, element, attrs, ngModel) {
	            ...
	            /* Fixs */
	            scope.$watch('change', function (nVal) {
	                element.val(nVal);
	            });

				element.bind("keydown keypress", function () {
	                var currentValue = element.val();
	                if (scope.change !== currentValue) {
	                    scope.$apply(function () {
	                        scope.change = currentValue;
	                    });
	                }
	            });

				//change element value 
	 			scope.elementValueChange = function (e) {
	                var currentValue = e;
	                if (scope.change !== currentValue) {
	                    scope.change = currentValue;
	                    var selectedDate = moment(currentValue, 'DD.MM.YYYY');
	                    ngModel.$setViewValue(currentValue);
	                    scope.viewValue = currentValue;
	                    scope.dateValue = selectedDate._d;
	                    scope.closeCalendar();
	                }
	            }
			...
			template: 
			'<div>' +
			'	<input id="datepickerInput-{{$id}}" type="text" tabindex="1" ng-focus="showCalendar()" ng-change="elementValueChange(viewValue)" ng-model="viewValue" ng-blur="blurHandler()" class="ng-datepicker-input" placeholder="{{ placeholder }}">' +
			'</div>' +
